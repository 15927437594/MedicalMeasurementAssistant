...
/**
 * user: Created by DuJi on 2021/10/7 17:54
...
public class CurveChartView extends View {
     ...
     * @param pointY1
     */
    public void setPointsY(final List<Double> pointY1) {
        if (null != pointY1) {
            if (!haveFocuse)
    ...
    private void setYDatas(final List<Double> pointY1) {
        pointY = pointY1;
     ...
     * @return
     */
    public double transformYCoordinate(double y) {
    ...
    private void drawCurveChart(Canvas canvas) {
        ...
        if (null != calculate_x && null != calculate_y && calculate_y.size() >= calculate_x.size()) {
            float eval = (float) calculate_x.get(0).eval(0);
            float eval1 = (float) calculate_y.get(0).eval(0);
            curvePath.moveTo(eval, eval1);

            for (int i = 0; i < calculate_x.size(); i++) {
                for (int j = 1; j <= STEP; j++) {
                    float u = j / (float) STEP;
                    float eval2 = (float) calculate_x.get(i).eval(u);
                    float eval3 = (float) calculate_y.get(i).eval(u);
                    curvePath.lineTo(eval2, eval3);
                }
            }
        }
     ...
     * @return
     */
    private List<Cubic> calculate(List<Double> x) {
        if (null != x && x.size() > 0) {
            int n = x.size() - 1;
